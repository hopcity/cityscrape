...
do
	...
	#shp2pgsql -I -s 6269 $f | psql -h localhost -U postgres -d city
done
...
do
	...
	do
		echo "[File: "$f" ]	[Table - "$i"]"
		#mdb-export -D '%%Y-%%m-%%d %%H:%%M:%%S' -q -I postgres $f $i | psql -h localhost -U postgres -d city #> /dev/null 2>&1
		mdb-export -D ‘%%Y-%%m-%%d %%H:%%M:%%S’ -I postgress -q \’ -R \; $f $i | psql -d city -U postgres -w -h localhost
#		 mdb-export -D '%%Y-%%m-%%d %%H:%%M:%%S' -Q $f $i > "$i.csv"
		#mdb-export -I postgres -q \' $f $i | psql -d city -U postgres -w -h localhost
